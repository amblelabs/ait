
<div align="center">

### **Для вас від [Amble Labs](https://amblelabs.github.io)**
<img src="src/main/resources/assets/ait_logo_1400px.png" alt="AIT Logo" width="700" />


&nbsp;

[<img alt="curseforge" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/available/curseforge_vector.svg">](https://www.curseforge.com/minecraft/mc-mods/adventures-in-time) <!-- SVG version -->
[<img alt="modrinth" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/available/modrinth_vector.svg">](https://modrinth.com/mod/ait) <!-- SVG version -->
[<img alt="fabric" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/supported/fabric_vector.svg">](https://fabricmc.net/) <!-- SVG version -->
[<img alt="forge" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/supported/forge_vector.svg">](https://files.minecraftforge.net) <!-- SVG version -->
[<img alt="amble" height="56" src="https://raw.githubusercontent.com/amblelabs/modkit/refs/heads/main/promo/cozy_vector_requires.svg">](https://modrinth.com/mod/amblekit)

</div>

<h2>
  <img src="https://cdn.modrinth.com/data/cached_images/8ad66df4dbb9eb7105e700a110a8350e691277cb.png"
       alt="Попередження"
       width="25"
       height="25"
       style="vertical-align: middle; margin-right: 8px;">
  Попередження
</h2>

**ЯКЩО ВИ НЕ МАЄТЕ ГП NVIDIA, НЕОБХІДНО ВИКОРИСТОВУВАТИ INDIUM ДЛЯ ЕФЕКТУ "БВНЗ", ЯКЩО ЦЕ НЕ ВИПРАВЛЯЄ ПРОБЛЕМУ, МИ НЕ МОЖЕМО З ЦИМ НІЧОГО ЗРОБИТИ**

**Версії `1.2.5.2812-1.20.1-release` й вище потребують [YACL](https://modrinth.com/mod/yacl/versions?g=1.20.1&l=fabric).**

**Версія `1.0.5-1.20.1-release` й її варіант для Forge дуже багована, особливо в одиночній грі, будьте обережні.**

## Загальне
**Apparatus Vivi. Жива машина. ТАРДІС. Неважливо, як ви її називаєте, вона росте й навчається разом із вами.
Вона вчитиметься в вас, тож поводіться з нею добре, інакше вона може навчитись тому, чому ви би не хотіли її вчити.
Також мод додає космос, Марс, й Місяць, та ще більше, аби ви могли подорожувати з вашою ТАРДІС. 
Майте на увазі: Вона жива, тож не тільки ви єдиний пілот...**

![Apparatus Vivi](https://cdn.modrinth.com/data/cached_images/dc30c41ab2a793e2bfedef625ba37118e7950a07_0.webp)

<h2>
  <img src="https://cdn.modrinth.com/data/cached_images/b18b275a0e9bb4000e015b935b65037166301538.png"
       alt="Що додає цей мод?"
       width="25"
       height="25"
       style="vertical-align: middle; margin-right: 8px;">
  Що додає цей мод?
</h2>

* Функціонуючу ТАРДІС
* 3 виміри (космос, Марс та Місяць)
* 35+ функціонуючих елементів керування
* Звуковий дизайн, що справді захоплює
* Унікальні елементи керування для інтер'єрів (Кавомашина)
* Вихор часу, що можна налаштувати під себе
* Розломи в часі
* Систему рушія
* Анімації, що можна налаштувати на своє уподобання
* Вечіркову ТАРДІС! (Покладіть диск у порт консолі/Назвіть вашу ТАРДІС "partytardis")
* Звукову Викрутку
* Зіркову Мапу (Локатор)
* Ручки
* Примарний монумент 
* Фазування рушіїв
* Підтримку шейдерів
* Божевільна кількість налаштувань
* Й більше...

[<img alt="minecraft storage" height="56" src="https://cdn.modrinth.com/data/cached_images/bd0e022e946783d3b0b9a68c1acdc4ffec7505c3_0.webp">](https://minecraftstorage.com/mods/adventures-in-time)

Доступно для Fabric + Forge на 1.20.1

Рекомендується **мінімум** 8ГБ пам'яті для одиночної гри.
Рекомендується **мінімум** 8ГБ **виділено** для сервера.

<h2>
  <img src="https://cdn.modrinth.com/data/cached_images/29848556cea889595907388e4cb59a97a4e86aea.png"
       alt="Треба довідка? Застряг? Не знаєш, для чого потрібно щось?"
       width="25"
       height="25"
       style="vertical-align: middle; margin-right: 8px;">
  Треба довідка? Застряг? Не знаєш, для чого потрібно щось?
</h2>

Якщо ви не розумієте щось щодо спеціальних блоків, гляньте на підказку, аби дізнатись більше про умови,
яких треба дотриматись аби ці блоки поставити. Ми також маємо [вікі](https://amblelabs.github.io/ait-wiki), що документує
більшість речей, що пов'язані із модом: від блоків й речей до того, як створити власний датапак для екстер'єрів та консолей.

![Звукова викрутка дає дізнатись, що робить елемент керування!](https://cdn.modrinth.com/data/cached_images/e9a7d7247bd2c74f8690fa99caf30acda381c37d.png)

<h2>
  <img src="https://cdn.modrinth.com/data/cached_images/808c7934614530076d21dd0cf5c5e2e992595985.png"
       alt="Сумісність"
       width="25"
       height="25"
       style="vertical-align: middle; margin-right: 8px;">
  Сумісність
</h2>

### [Gravity Changer (форк qouteall)](https://modrinth.com/mod/gravity-api-fork/versions)
- Ви зможете контролювати гравітацію всередині ТАРДІС за допомогою [Монітора](https://loqor.dev/ait/Monitor/)

Ми також маємо багато інших варіантів сумісності, але вам буде необхідно встановити окремий, спеціальний мод для AIT, що дозволить їх розблокувати.

### [Adventures in Space](https://github.com/DrTheodor/adventures-in-space) та [Ad Astra](https://modrinth.com/mod/ad-astra)
- Особлива сумісність для ТАРДІС під час гри із Ad Astra

### [Immersive Portals](https://modrinth.com/mod/immersiveportals)

- Ваша ТАРДІС буде "БВНЗ" - Більше Всередині, Ніж Ззовні
- З відчиненими дверима ви бачитимете інтер'єр ззовні й навпаки

<h2>
  <img src="https://cdn.modrinth.com/data/cached_images/7412fc34a0142c5cc1ec9eee18c68c81fbbb4d81.png"
       alt="Подяки"
       width="25"
       height="25"
       style="vertical-align: middle; margin-right: 8px;">
  Подяка
</h2>



### Розробники:
- [Loqor](https://loqor.dev)
- [Dr. Theo](https://theo.is-a.dev/)
- [duzo](https://duzo.is-a.dev/)
- [Maketendo](https://github.com/MaketendoDev)
- [AddieDaBaddie](https://addi3.github.io/index)
- [Saturn](https://github.com/Saturnorsomthing)
- [Monke](https://github.com/rapbattlegod32)
- [Pan](https://github.com/Mansarde)
- і більше! (Перевірте [GitHub](https://github.com/amblelabs/ait/graphs/contributors) для повного списку учасників)
---

### Артисти:
- WazzakiWanzz
- Westankrang
- Tright
- b2
- Ouro
- Maggie
- Theta

---

### Звуковий дизайн:
- RatZoomie
- Dian
- TARDISSFX (the YouTube channel)
- Uriel_N_Diaz 
- Classic
  
---

### Окрема вдячність:
- [MagicMan(?)](https://magicmaan.github.io/) (Допомога із 3D-моделями)
- Bug1312 (Допомога із кодом)
- DocArcane (Стимпанк консоль)
- Creativious (Допомога із кодом)
- nitrogenez (Український переклад)
- Codiak (Допомога із кодом)

---

<h2>
  <img src="https://cdn.modrinth.com/data/cached_images/23b97ecfe49586f70c6a7d4e4ca63ac14d47e6e1.png"
       alt="Посилання й спільнота"
       width="25"
       height="25"
       style="vertical-align: middle; margin-right: 8px;">
  Посилання й спільнота
</h2>

<div style="text-align: center; font-size: 1.5em; margin-top: 1em; white-space: nowrap;">
  <a href="https://amblelabs.github.io/ait-wiki/" style="text-decoration: none; color: inherit; display: inline-block; margin: 0 8px;">Вікі</a>
  <span style="display: inline-block; margin: 0 4px;">•</span>
  <a href="https://github.com/amblelabs/ait/" style="text-decoration: none; color: inherit; display: inline-block; margin: 0 8px;">GitHub</a>
  <span style="display: inline-block; margin: 0 4px;">•</span>
  <a href="https://www.curseforge.com/minecraft/mc-mods/adventures-in-time" style="text-decoration: none; color: inherit; display: inline-block; margin: 0 8px;">CurseForge</a>
  <span style="display: inline-block; margin: 0 4px;">•</span>
  <a href="https://modrinth.com/mod/ait" style="text-decoration: none; color: inherit; display: inline-block; margin: 0 8px;">Modrinth</a>
  <span style="display: inline-block; margin: 0 4px;">•</span>
  <a href="https://discord.com/invite/WjKhRjavCj" style="text-decoration: none; color: inherit; display: inline-block; margin: 0 8px;">Discord</a>
</div>

&nbsp;

![Приєднуйтеся до нашого сервера в Minecraft!](https://cdn.modrinth.com/data/cached_images/2c9f228e916187fcc47e6ad92e09ab9ad52c1949.png)

